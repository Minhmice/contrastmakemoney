...
function ProductCard({ product, selected, onToggle }: { product: MenuProduct; selected: boolean; onToggle: (product: MenuProduct) => void }) {
  ...
  return <Card className="menu-product-shell" data-product-id={product.id} data-selected={selected || undefined}><button type="button" className="menu-product-card" onClick={() => onToggle(product)} aria-expanded={selected} aria-controls={detailId}><span className="menu-product-card__indicator" aria-hidden="true" /><span className="menu-product-card__visual"><ProductImage product={product} /></span><CardContent className="menu-product-card__body"><span className="menu-product-card__name">{product.nameVi}</span><span className="menu-product-card__english">{product.nameEn}</span><span className="menu-product-card__price">{sizes(product).map((size) => price(product.prices[size] ?? 0)).join(' / ')}</span><span className="menu-product-card__detail">{selected ? 'ĐANG XEM' : 'XEM CHI TIẾT'} →</span></CardContent></button></Card>
}

function InlineDetail({ product, selectedSize, onClose, onSize, onOrder, pending }: { product: MenuProduct; selectedSize: MenuSize; onClose: () => void; onSize: (size: MenuSize) => void; onOrder: () => void; pending: boolean }) {
  return <article id={'menu-detail-' + product.id} className="menu-inline-detail" data-detail-for={product.id}><button type="button" className="menu-inline-detail__close" onClick={onClose} aria-label="Đóng chi tiết thức uống"><X size={18} /></button><div className="menu-inline-detail__content"><header><h3>{product.nameVi}</h3><p className="menu-inline-detail__english">{product.nameEn}</p><p className="menu-inline-detail__notes">{product.characteristics.join(' · ')}</p><p className="menu-inline-detail__description">{product.description}</p></header><fieldset className="menu-size-selector"><legend>CHỌN SIZE</legend><div>{sizes(product).map((size) => <button type="button" key={size} data-selected={size === selectedSize || undefined} aria-pressed={size === selectedSize} onClick={() => onSize(size)}><span>{size}</span><strong>{price(product.prices[size] ?? 0)}</strong></button>)}</div></fieldset><button type="button" className="menu-inline-detail__order" onClick={onOrder} disabled={pending}>{pending ? 'ĐANG LƯU...' : 'ORDER - ' + price(product.prices[selectedSize] ?? 0)} <span>→</span></button></div></article>
}
...
export default function MenuPage({ user }: { user: User | null }) {
  ...
  const closeDetail = () => { setSelected(null); setSelectedSize(null) }
  const toggle = (product: MenuProduct) => { if (selected === product.id) closeDetail(); else { setSelected(product.id); setSelectedSize(sizes(product)[0] ?? 'M') } }
